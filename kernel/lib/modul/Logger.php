...
class Logger{
  ...
  public static function log($data, $type="INFO", $file="main" , $flush = false){
    ...
    if(file_exists($log_path)){
      ...
      if(defined("LANGAGE")){
        echo LANGAGE == "FR" ? "<b>Impossible de trouver le fichier de log. Chemin utilisé : '".$log_path."'. Nom du fichier voulu : '".$file.".log'.Avez vous pensez à re-définir la variable 'log_path' au début de la librairie ?</b>" : "<b>Cannot find log file. Path used : '".$log_path."'. Did you changed the log_path variable at the top of the librarie ?</b>";
      }
    }
    return(false);
  }
